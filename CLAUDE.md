...
`.github/workflows/publish.yaml` runs on every GitHub Release (`types: [published]`) and dual-publishes the built package: `@robineb/project-cli` to npmjs.com (anonymous `npm i -g @robineb/project-cli`, no auth needed to install) and, in a separate parallel job, `@robin1053/project-cli` to GitHub Packages (auth via the built-in `GITHUB_TOKEN`, needs `permissions: packages: write`) purely so the repo's GitHub sidebar shows a Packages widget — installing from GitHub Packages requires a PAT even for public packages, so npmjs.com stays the actual distribution channel. The GitHub Packages job renames the package via `npm pkg set name="@robin1053/project-cli"` in the runner's checkout only; the committed `package.json` keeps the npmjs.com name (`@robineb/project-cli`).

**The package name is scoped (`@robineb/...`), not the plain `project-cli` you'd expect.** The unscoped name is already taken by an unrelated package on npmjs.com — found out by hitting a `403 Forbidden` on the very first real publish attempt. The `bin` entry (the actual command name, `project-cli`) is independent of the package name, so this doesn't change what users type.

The npmjs.com job authenticates via **Trusted Publishing (OIDC)**, not a stored token: `permissions: id-token: write` on that job lets `npm publish` exchange GitHub's OIDC identity for a short-lived publish credential, provided npmjs.com has a Trusted Publisher configured for the package pointing at this exact repo + workflow filename. Deliberately not using a classic/granular npm access token: npm is deprecating direct-publish tokens in January 2027 and steers CI/CD toward Trusted Publishing instead (confirmed live in npm's own token-creation UI while setting this up), and granular write tokens now cap out at a 90-day expiry anyway, which would mean manually rotating a `NPM_TOKEN` secret every few months.

**Trusted Publishing can only be configured for a package that already exists on the registry** (confirmed in `npm trust`'s own docs — not obvious going in, cost a wasted round of "just set it up first" advice). So the very first publish has to happen manually, once, outside CI: `npm run build && npm login && npm publish`, then `npm trust github --allow-publish --file publish.yaml` (needs local npm ≥ 11.15.0; auto-detects owner/repo from the `repository` field). Every release after that goes through the CI job with no secret at all.
